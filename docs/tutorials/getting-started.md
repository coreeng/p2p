...
Fast-feedback also calls source security scanning and image scanning automatically on each pull request and push. Source security scanning covers source dependency vulnerabilities, restricted or forbidden licenses, and git-tree secrets. Image scanning covers both known CVEs and embedded secrets. At this level, `security-scan-fail-on-findings` defaults to `false`, so the scans report findings without blocking by default.
