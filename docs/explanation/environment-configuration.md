...
The action reads the platform variables already present in the job environment (from GitHub Environment settings) and computes derived authentication variables (`REGISTRY`, `SERVICE_ACCOUNT`, `WORKLOAD_IDENTITY_PROVIDER`) and the full set of P2P convenience variables (`P2P_TENANT_NAME`, `P2P_APP_NAME`, `P2P_VERSION`, registry paths, and namespace variants). These match the variables that `p2p.mk` produces, so Makefiles work identically whether they run locally or in CI.
