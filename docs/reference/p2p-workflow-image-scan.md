...
| `app-name` | string | No | `''` | Application name used to scope sticky PR comments in multi-app repositories. When empty, the workflow uses the legacy unscoped comment header. |
...
- A sticky PR comment with `header: image-scan-findings[-<app-name>]-<stage>-<github_env>` on `pull_request` events when `dry-run: false` and the caller grants `pull-requests: write`. When `app-name` is empty, the app segment is omitted; when `github_env` is empty, the header uses `local`.
