...
| `repository-runner-setup` | `boolean` | No | `false` | When `true`, each command job runs the caller repository's `.github/actions/p2p-runner-setup` composite action after checkout, passes its current command, and lets that action own Docker builder setup. |
...
`runner-label` and `repository-runner-setup` are forwarded only to the command-execution jobs (`build`, `functional-test`, `nft-test`, and `integration-test`). When repository setup is enabled, the caller repository must provide `.github/actions/p2p-runner-setup/action.yaml` or `.github/actions/p2p-runner-setup/action.yml`. The composite action must declare a required `command` input and receives `p2p-build`, `p2p-functional`, `p2p-nft`, or `p2p-integration` for the corresponding job.
