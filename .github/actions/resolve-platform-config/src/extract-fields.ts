...
export function extractAndExportFields(config: EnvironmentConfig, inputs: ActionInputs): void {
  const fields = inputs.fields === 'full' ? [...CORE_FIELDS, ...FULL_FIELDS] : CORE_FIELDS;

  for (const { envVar, path } of fields) {
    const value = getNestedValue(config, path);
    const source = inputs.configMode === 'repo-file' ? inputs.repoFilePath : inputs.centralRepoName;
