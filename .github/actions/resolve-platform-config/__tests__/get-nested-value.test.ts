...
describe('getNestedValue', () => {
  const config = {
    platform: { projectId: 'my-project', projectNumber: '123456', region: 'europe-west2' },
  ...
  it('resolves array bracket notation', () => {
    expect(getNestedValue(config, 'ingressDomains[0].domain')).toBe('dev.example.com');
  });

  it('resolves nested object paths', () => {
    expect(getNestedValue(config, 'internalServices.domain')).toBe('internal.dev.example.com');
  });
