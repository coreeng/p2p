...
describe('validateInputs', () => {
  ...
  it('rejects central-repo mode without central-repo-name', () => {
    expect(() =>
      validateInputs(makeInputs({
        configMode: 'central-repo', centralRepoName: '', centralRepoOwner: 'coreeng', centralRepoToken: 'ghp_token',
      }))
  ...
  it('rejects central-repo mode without central-repo-owner', () => {
    expect(() =>
      validateInputs(makeInputs({
        configMode: 'central-repo', centralRepoName: 'config-repo', centralRepoOwner: '', centralRepoToken: 'ghp_token',
      }))
    ).toThrow("config-mode is 'central-repo' but central-repo-owner is not set");
  });

  it('rejects central-repo mode without central-repo-token', () => {
    expect(() =>
      validateInputs(makeInputs({
        configMode: 'central-repo', centralRepoName: 'config-repo', centralRepoOwner: 'coreeng', centralRepoToken: '',
      }))
    ).toThrow("config-mode is 'central-repo' but central-repo-token is not set");
  });

  it('rejects config inputs when config-mode is empty', () => {
    expect(() =>
      validateInputs(makeInputs({ repoFilePath: '.p2p.yaml' }))
    ).toThrow('config-mode is not set but repo-file or central-repo inputs are provided');
  });
  ...
  it('accepts fields=core', () => {
    expect(() => validateInputs(makeInputs({ fields: 'core' }))).not.toThrow();
  });
});
