...
helper="${script_dir}/../registry-login"
tmp_dir="$(mktemp -d)"
...
run_helper ipv4-loopback --audience workload --registry 127.0.0.1:5000 --skopeo-auth-file "${tmp_dir}/loopback-auth.json"
[[ ${RUN_STATUS} -eq 0 ]] || fail "IPv4 loopback invocation failed: $(cat "${RUN_OUTPUT_FILE}")"
assert_file_equals $'login\n127.0.0.1:5000\n--username\noauth\n--password-stdin' "${MOCK_DOCKER_ARGS_FILE}"
assert_file_equals $'login\n--tls-verify=false\n--authfile\n'"${tmp_dir}"$'/loopback-auth.json\n--username\noauth\n--password-stdin\n127.0.0.1:5000' "${MOCK_SKOPEO_ARGS_FILE}"
assert_login_token_confined 'raw-zot-oidc-token'
