...
assert_file_contains_line '--fail' "${MOCK_CURL_ARGS_FILE}"
assert_file_contains_line '--silent' "${MOCK_CURL_ARGS_FILE}"
assert_file_contains_line '--show-error' "${MOCK_CURL_ARGS_FILE}"
assert_file_contains_line '--location' "${MOCK_CURL_ARGS_FILE}"
assert_file_contains_line 'Authorization: Bearer request-token' "${MOCK_CURL_ARGS_FILE}"
assert_file_contains_line 'https://tokens.example.test/oidc?audience=api%3A%2F%2Fcluster%20name%3Fx%3Dy%26z%3D%2F%2B' "${MOCK_CURL_ARGS_FILE}"
...
assert_file_contains_line 'https://tokens.example.test/oidc?api-version=1&audience=workload%20identity' "${MOCK_CURL_ARGS_FILE}"
